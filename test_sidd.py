...
# from process_sidd.process import process_sidd_validation
...
for valid_name, valid_data in valid_dict.items():
    ...
    for idx in range(num_img):
        for idy in range(num_block):
            im = valid_gt[idx, idy]
            noisy_im = valid_noisy[idx, idy]
            noisy_tensor = noisy_im[:, :, np.newaxis]

            origin255 = im[:, :, np.newaxis].copy() * 255.0
            origin255 = origin255.astype(np.uint8)
            noisy255 = noisy_tensor.copy() * 255.0
            noisy255 = noisy255.astype(np.uint8)

            transformer = transforms.Compose([transforms.ToTensor()])
            noisy_tensor = transformer(noisy_tensor)
            noisy_tensor = torch.unsqueeze(noisy_tensor, 0)
            noisy_tensor = noisy_tensor.cuda()
            # pack raw data
            noisy_tensor = space_to_depth(noisy_tensor, block_size=2)
            with torch.no_grad():
                n, c, h, w = noisy_tensor.shape
                exp_output = network(noisy_tensor)
            ...
            pred_exp = np.squeeze(pred_exp)
            # print(im.shape, pred_exp.shape)
            pred255_exp = np.clip(pred_exp * 255.0 + 0.5, 0,
                                255).astype(np.uint8)
            psnr_noisy = calculate_psnr(im.astype(np.float32), noisy_im.astype(np.float32), 1.0)
            ssim_noisy = calculate_ssim((im*255.0).astype(np.float32), (noisy_im*255.0).astype(np.float32))
            ...
            logger.info(
                "{} - img:{}_{:03d} - PSNR_EXP: {:.6f} dB; SSIM_EXP: {:.6f}.".format(
                valid_name, idx, idy, psnr_exp, ssim_exp
                )
            )
            
            # # visualization
            # out_gt = process_sidd_validation(im)
            # for camera, out_img in out_gt.items():
            #     save_path = os.path.join(save_dir, "{:03d}-{:03d}-clean-{}.png".format(idx, idy, camera))
            #     # print(save_path)
            #     Image.fromarray(out_img).save(save_path)
            # out_noisy = process_sidd_validation(noisy_im)
            # for camera, out_img in out_noisy.items():
            #     save_path = os.path.join(save_dir, "{:03d}-{:03d}-noisy-{}-{:0.2f}-{:0.3f}.png".format(idx, idy, camera, psnr_noisy, ssim_noisy))
            #     Image.fromarray(out_img).save(save_path)
            # out_prediction = process_sidd_validation(pred_exp)
            # for camera, out_img in out_prediction.items():
            #     save_path = os.path.join(save_dir, "{:03d}-{:03d}-denoised-{}-{:0.2f}-{:0.3f}.png".format(idx, idy, camera, psnr_exp, ssim_exp))
            #     Image.fromarray(out_img).save(save_path)
    ...
    logger.info(
        "----Average PSNR/SSIM results for {}----\n\t----PSNR_EXP: {:.6f} dB; SSIM_EXP: {:.6f}\n".format(
            valid_name, avg_psnr_exp, avg_ssim_exp
        )
    )